...
{
    private const float AVERAGE_VELOCITY = 0.1f;
    ...
    void Start()
    {
        ORIGIN = GameObject.Find("instantiateHere").transform.position;
        ...
        _velocity = _seed * 0.5f + 0.125f * Random.value;
        _vertical_drift = (_seed - 0.5f) * 0.05f * Random.value;
    }
    ...
    private void UpdatePosition()
    {
        ...
        //interpolate the values for the new direction by how far the Butterfly is from the edges of the origin
        ...
        //this points them along a course parallel to the ground, with each Butterfly having a tendency to drift up or down as it goes
        ...
        float velocity_swoop = -_direction.y * _velocity * 0.08f;
        ...
        //look towards where the Butterfly is flying
        gameObject.transform.LookAt(gameObject.transform.position + _direction);


        //move the Butterfly forward along that path
